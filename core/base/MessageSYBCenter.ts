 ...
 * 服务端返回消息处理
 */
class MessageSYBCenter extends BaseSYBClass {
	private dict: any;
	private eVec: MessageVo[];
	private type: number;
	private flag: number = 0;

	public static ins(): MessageSYBCenter {
		// if (!MessageSYBCenter._ins) MessageSYBCenter._ins = new MessageSYBCenter(0);
		// return MessageSYBCenter._ins;
		return super.ins(0) as MessageSYBCenter;
	}
	...
	public dispatch(type: string, ...param: any[]): void {
		...
		if (this.type == 0) {
			...
			Log.trace("MessageSYBCenter未实现的类型");
		}
	}
	...
	public static setFunction(ins: boolean, obj: any, name: string, ex: string): boolean {
		if (name.indexOf(ex) == 0 && typeof (obj[name]) == "function") {
			let msgname = egret.getQualifiedClassName(obj) + MessageSYBCenter.splite + name + MessageSYBCenter.msgIndex;
			MessageSYBCenter.msgIndex += 1;
			...
			let newfunc = function (...args: any[]) {
				...
				if (typeof data != "boolean" || data) {
					MessageSYBCenter.ins().dispatch(msgname, data);
				}
				return data;
			};
	...
	public static compile(thisobj: any, ex: string = "post") {
		...
		for (let name in p) {
			MessageSYBCenter.setFunction(true, p, name, ex);
		}

		// for (let name in thisobj) {
		// 	MessageSYBCenter.setFunction(false, thisobj, name, ex);
		// }
	...
	public static addListener(func: any, listener: Function, thisObj: any, callobj: any = undefined): boolean {
		if (func.funcallname) {
			MessageSYBCenter.ins().addListener(func.funcallname, listener, thisObj);
			if (callobj)
				func.call(callobj);
			return true;
		}
		else {
			//debug.log("MessageSYBCenter.addListener error:" + egret.getQualifiedClassName(thisObj));
			return false;
		}
	}
}
...
function post(target, key: string, descriptor: PropertyDescriptor) {
	...
	let msgname = target.constructor.name + MessageSYBCenter.splite + key;
...
function callDelay(delay:number) {
	let func = function (target, key: string, descriptor: PropertyDescriptor) {
		...
		let newfunc2 = function (...args) {
			...
			// TimerSYBManager.ins().doTimer(delay, 1, newfunc.bind(this, ...args), this); //有隐患，假如TimerSYBManager.ins().removeAll(this)之后，可能会出现异常
		};
