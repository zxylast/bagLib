 ...
 * Timer管理器
 */
class TimerSYBManager extends BaseSYBClass {
	private _handlers: Array<TimerHandler>;
	public _conTime:number;
	...
	public constructor() {
		...
		this._conTime = this._currTime = egret.getTimer();
		this._currFrame = 0;

		egret.startTick(this.onEnterFrame, this);
	}

	public static ins(): TimerSYBManager {
		return super.ins() as TimerSYBManager;
	}
	 ...
	 * CDMs 毫秒
	*/
	public getCDing(key, CDMs): boolean {
		if(this.cdPool[key]){
			return egret.getTimer() - this.cdPool[key] < CDMs;
		}
		return false;
	}
	/**
	...
	*/
	public setCD(key) {
		this.cdPool[key] = egret.getTimer();
	}
	...
	private onEnterFrame(time: number): boolean {
		...
		if (nexthandles && nexthandles.length > 0) {
			for (let handler of nexthandles) {
				...
				TimerSYBManager.DeleteHandle(handler);
			}
		...
		while (handler.exeTime <= this._currTime) {
			...
			if (repeat) {
				let index = Algorithm.binSearch(this._handlers, handler, TimerSYBManager.binFunc);
