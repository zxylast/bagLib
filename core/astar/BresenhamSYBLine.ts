...
//			trace(throughCheckResult[2], throughCheckResult[3]);
		...
		for (i = 0; i < dx; i++) {
			...
			while (lastY != y) {
				lastY += ystep;
				// if (!GameSYBMap.checkWalkable(x - xstep, lastY)) return this.throughCheckResult;
				// if (!GameSYBMap.checkWalkable(x, lastY)) return this.throughCheckResult;
				// this.throughCheckResult[2] = x - xstep, this.throughCheckResult[3] = lastY;
				this.throughCheckResult[2] = x, this.throughCheckResult[3] = lastY;
//					trace(throughCheckResult[2], throughCheckResult[3]);
			}
			//押入当前计算得出的格子
			// if (!GameSYBMap.checkWalkable(x, y)) return this.throughCheckResult;
...
//				trace(throughCheckResult[2], throughCheckResult[3]);
	...
	public static shortenPath(ox: number, oy: number, path: number[], distance: number): void {
		...
		if (dx * dx + dy * dy > distance * distance) {
			pt = BresenhamSYBLine.getPointOnLine(sx, sy, tx, ty, -distance);
