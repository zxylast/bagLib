 ...
 * Socket类
 */
class Socket extends BaseSYBClass {
	...
	private onSocketOpen():void {
		...
		if (this._connectFlag) {
			MessageSYBCenter.ins().dispatch(SocketConst.SOCKET_RECONNECT);
		} else {
			MessageSYBCenter.ins().dispatch(SocketConst.SOCKET_CONNECT);
		}
	...
	private onSocketClose():void {
		if (this._needReconnect) {
			this.reconnect();
			MessageSYBCenter.ins().dispatch(SocketConst.SOCKET_START_RECONNECT);
		} else {
			MessageSYBCenter.ins().dispatch(SocketConst.SOCKET_CLOSE);
		}
	...
	private onSocketError():void {
		if (this._needReconnect) {
			...
			MessageSYBCenter.ins().dispatch(SocketConst.SOCKET_NOCONNECT);
		}
		this._isConnecting = false;
	}
	...
	private debugInfo(str:String):void {
		MessageSYBCenter.ins().dispatch(SocketConst.SOCKET_DEBUG_INFO, str);
	}
