...
class StringSYBUtils {
	...
	public static complementByChar(str: string, length: number, char: string = " ", ignoreHtml: boolean = true): string {
		let byteLen: number = this.strByteLen(ignoreHtml ? str.replace(StringSYBUtils.HTML, "") : str);
	...
	public static substitute(str: string, ...rest): string {
		let reg: RegExp = RegExpSYBUtil.REPLACE_STRING;
	 ...
	 * 中文转数字
	 * 例子:
	 * StringSYBUtils.ChineseToNumber(三百四十三) = 343 (number）
	 * */
	...
	public static ChineseToNumber(chnStr: string) {
		...
		for (let i = 0; i < str.length; i++) {
			let num = StringSYBUtils.chnNumCharCN[str[i]];
			if (typeof num !== 'undefined') {
				...
				let unit = StringSYBUtils.chnNameValueCN[str[i]].value;
				secUnit = StringSYBUtils.chnNameValueCN[str[i]].secUnit;
				if (secUnit) {
	 ...
	 * 数字转中文
	 * 例子:
	 * StringSYBUtils.NumberToChinese(325) = "三百二十五" (string）
	 * */
	...
	public static NumberToChinese(num: number) {
		...
		let strIns = '', chnStr = '';
		let needZero = false;
		let chnNumChar = StringSYBUtils.chnNumChar;
		let chnUnitSection = StringSYBUtils.chnUnitSection;
		if (num === 0) {
			return chnNumChar[0];
		}
		...
		while (num > 0) {
			...
			strIns = StringSYBUtils.SectionToChinese(section);
	...
	private static SectionToChinese(section: number) {
		...
		let zero = true;
		let chnNumChar = StringSYBUtils.chnNumChar;
		let chnUnitChar = StringSYBUtils.chnUnitChar;
		while (section > 0) {
			let v = section % 10;
			if (v === 0) {
				if (!zero) {
	 ...
	 * @returns string 文本
	 */
	public static replaceContent(str: string, datas: string[]): string {
