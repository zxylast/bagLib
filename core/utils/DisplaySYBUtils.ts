...
class DisplaySYBUtils {
	...
	public static shakeIt(target: egret.DisplayObject,
		...
		egret.Tween.get(target).to(
			...
			shakeSet[2], delay
				...
				DisplaySYBUtils.shakeIt(target, range, duration, --times);
			}, this
			);
	}
	...
	/**画扇形 */
	public static drawCir(shape: egret.Shape, radius: number, starAngle:number= 0, angle: number = 0, anticlockwise?: boolean): egret.Shape {
		if (shape == null) {
		...
		function changeGraphics(): void {
			...
			shape.graphics.drawArc(0, 0, radius, starAngle* Math.PI / 180, angle * Math.PI / 180, anticlockwise);
