...
class MathSYBUtils {
	...
	/**随机数组中的下标 */
	public static onArrayRandomSub(list: any[]): number {
		return Math.floor(Math.random() * list.length);
	}

	/**在一个数组中随多个不同的值，会改变原数组
     * @param rans 随机原数组
     * @param cnt 需要几位随机值--- 默认是1位
     * @param list 保存随机值的数组--- 默认空数组
     * @returns 返回数组--保存随机数组的下标
     */
	public static onArrayRandomMultiple(rans: any[], cnt: number = 1, list: any[] = []): any[] {
		if (!rans.length) return list;
		let num: number = this.onArrayRandomSub(rans);
		list.push(rans[num]);
		rans.splice(num, 1);
		cnt--;
		if (cnt > 0) return this.onArrayRandomMultiple(rans, cnt, list);
		return list;
	}
