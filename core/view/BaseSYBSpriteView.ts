 ...
 */
class BaseSYBSpriteView extends egret.DisplayObjectContainer implements IBaseSYBView {
	...
	public constructor( $parent: egret.DisplayObjectContainer) {
		...
		this._isInit = false;
		StageSYBUtils.ins().getStage().addEventListener(egret.Event.RESIZE, this.onResize, this);
	}
	...
	public loadResource(loadComplete: Function, initComplete: Function): void {
		if (this._resources && this._resources.length > 0) {
			ResourceSYBUtils.ins().loadResource(this._resources, [], function (): void {
				loadComplete();
				initComplete();
			}, null, this);
		}
